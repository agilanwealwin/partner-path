...
function WelcomePage() {
  ...
  return (
    <div className="p-4 md:p-10 max-w-[1200px] mx-auto space-y-8 md:space-y-16 bg-background">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-border pb-6 md:pb-8 gap-4">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <span className="hidden sm:inline-block px-2 py-0.5 rounded bg-surface-3 text-[10px] font-mono text-muted-foreground border border-border tracking-tighter">v1.2 // PROD</span>
            <h1 className="font-display font-bold text-xl sm:text-2xl md:text-3xl text-foreground tracking-tight leading-tight">Infrastructure Partner Programme</h1>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-4">
          <Button variant="outline" size="sm" className="flex-1 sm:flex-none h-10 px-4 sm:px-6 rounded-xl border-border bg-surface-2 hover:bg-surface-3 transition-colors" onClick={() => navigate('/onboarding/infra-partner/scoring')}>
            Scoring Matrix <ArrowUpRight size={14} className="ml-2 opacity-50" />
          </Button>
          <Button size="sm" className="flex-1 sm:flex-none h-10 px-4 sm:px-6 bg-primary hover:bg-primary/90 text-white rounded-xl font-bold shadow-lg shadow-primary/10" onClick={handleBegin}>
            Begin Enrollment <ArrowRight size={14} className="ml-2" />
          </Button>
        </div>
      </div>
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-[1fr,auto] gap-8 lg:gap-16 items-center">
          <div className="space-y-6 md:space-y-8 text-center lg:text-left">
            <div className="space-y-3">
              <p className="text-[10px] sm:text-[11px] font-black tracking-[0.3em] text-primary uppercase animate-in slide-in-from-left-4 duration-500">Global Infrastructure Network</p>
              <h2 className="font-display font-black text-3xl sm:text-5xl md:text-6xl text-foreground leading-[1.1] tracking-tight">
                Certified Infrastructure<br />
                <span className="text-primary italic">Service Providers</span>
              </h2>
            </div>
            <p className="text-base sm:text-lg text-muted-foreground max-w-xl mx-auto lg:mx-0 leading-relaxed opacity-90">
              Direct participation in the $DLN infrastructure network. Deploy, manage, and scale infrastructure nodes across the global DeLEN network.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 sm:gap-6 pt-4 animate-in fade-in zoom-in duration-700 delay-300">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-background-2 bg-surface-3 ring-2 ring-background flex items-center justify-center text-[10px] font-black text-muted-foreground">
      ...
      <div className="space-y-8 sm:space-y-10 pt-4 md:pt-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between px-2 gap-4">
          <div className="space-y-2">
            <h3 className="font-display font-bold text-xl text-foreground tracking-tight">Pathway Selection</h3>
            <p className="text-sm text-muted-foreground">Choose the level that aligns with your operational deployment capacity.</p>
          </div>
          <div className="inline-flex w-fit items-center gap-2 px-4 py-2 rounded-xl bg-surface-2 border border-border text-[11px] text-muted-foreground font-bold shadow-sm">
          ...
          {tiers.map((tier, i) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "relative rounded-3xl p-6 sm:p-10 border-2 transition-all duration-500 cursor-pointer flex flex-col group overflow-hidden",
                  isSelected
                    ? "bg-surface border-primary shadow-2xl shadow-primary/10 md:-translate-y-2 md:scale-[1.02]"
                    : "bg-surface-2/30 border-border hover:border-border-strong shadow-sm hover:shadow-md md:hover:-translate-y-1"
                )}
              >
                {/* Decorative Background Number */}
                <div className="absolute -bottom-10 -right-4 text-[10rem] sm:text-[12rem] font-display font-black text-foreground/[0.03] pointer-events-none group-hover:text-foreground/[0.06] transition-all duration-700 select-none">
                  {i + 1}
                </div>

                {tier.recommended && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 sm:px-5 py-1.5 sm:py-2 rounded-full bg-primary text-white text-[9px] sm:text-[10px] font-bold tracking-[0.2em] shadow-lg shadow-primary/20 z-20 uppercase">
                    RECOMMENDED
                  </div>
                )}
                ...
                <div className={cn(
                  "text-[9px] font-bold tracking-[0.25em] px-3 py-1.5 rounded-lg border-2 uppercase mb-8 sm:mb-12 w-fit transition-all text-center",
                  isSelected
                ...
                <div className="space-y-1 mb-6 sm:mb-10">
                  <p className={cn("text-[10px] sm:text-[11px] font-black tracking-[0.25em] uppercase mb-4", tier.color)}>{tier.name}</p>
                  <div className="h-0.5 w-12 bg-border group-hover:w-full transition-all duration-700 my-6 sm:my-8" />
                  <p className="text-[9px] text-muted-foreground uppercase font-bold tracking-widest opacity-60 mb-1 sm:mb-2">One-Time Partnership Fee</p>
                  <p className={cn("text-3xl sm:text-4xl font-display font-black tracking-tight", isSelected ? "text-foreground" : "text-foreground/80")}>{tier.fee}</p>
                </div>

                <div className="space-y-4 sm:space-y-5 flex-1 relative z-10">
                  {[
                    ...
                    <div key={feat.label} className="flex gap-4 items-start">
                      ...
                      <div className="space-y-1">
                        <p className="text-[9px] text-muted-foreground uppercase font-black tracking-widest opacity-70 group-hover:opacity-100 transition-opacity">{feat.label}</p>
                ...
                <Button
                  ...
                  className={cn(
                    "w-full mt-8 sm:mt-12 rounded-2xl h-12 sm:h-14 font-bold text-xs tracking-widest uppercase transition-all z-10",
                    isSelected
                ...
                >
                  {isSelected ? "Current Selection" : "Identity As Level " + (i + 1)}
                </Button>
              </div>
            );
          })}
      ...
      <div className="flex flex-col items-center gap-6 md:gap-8 pt-8 md:pt-16 border-t border-border mt-8 md:mt-16 animate-in fade-in slide-in-from-bottom-8">
        <div className="text-center space-y-3 px-4">
          <h4 className="font-display font-black text-2xl sm:text-3xl text-foreground">Ready to start?</h4>
          <p className="text-muted-foreground text-sm max-w-lg mx-auto">Proceeding will initiate the digital verification of your technical profile.</p>
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 w-full max-w-2xl px-4">
          <Button size="lg" className="h-16 w-full sm:flex-1 bg-primary hover:bg-primary/90 text-white rounded-2xl text-base font-black shadow-2xl shadow-primary/30 group transition-all md:hover:scale-[1.05]" onClick={handleBegin}>
            Begin Assessment <ArrowRight size={20} className="ml-3 group-hover:translate-x-2 transition-transform" />
          </Button>
          <Button variant="outline" size="lg" className="h-16 w-full sm:w-auto px-10 border-2 border-border text-foreground hover:bg-surface-2 rounded-2xl text-base font-black transition-all" onClick={() => navigate('/onboarding/infra-partner/support')}>
            View T&Cs
          </Button>
        </div>
        <p className="text-[11px] px-6 text-center text-muted-foreground italic flex flex-wrap items-center justify-center gap-2 opacity-60 font-medium leading-relaxed">
          <Info size={16} className="opacity-50 shrink-0" /> Secure digital ID required. DeLEN Infrastructure Review v2.4 applies.
        </p>
      </div>
    </div>
  );
